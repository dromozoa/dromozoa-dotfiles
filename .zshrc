# Copyright (C) 2018,2023,2024,2026 Tomoyuki Fujimori <moyu@dromozoa.com>
#
...
_dromozoa_prompt() {
  ...
  RPROMPT="%(?..${exit}[exit=%?]$reset )${date}[%D{%Y/%m/%d %H:%M:%S}]$reset"
...
_zsphre_preexec() {
  _zsphre_id=$(zsphre zsh_hook_preexec "$1" "$2" "$3" "$PWD" "$TTY" "$USER" "$HOST" || :)
}
